# Mysterious GIF

## 題目
從一張[神奇的gif](https://drive.google.com/file/d/0Bw-aSVx60ZiOU1h6QWRqMkNfOVU/view)中找出flag。

## Write-Up

一開始使用`binwalk Question.gif`檢查

```
DECIMAL       HEXADECIMAL     DESCRIPTION
--------------------------------------------------------------------------------
0             0x0             GIF image data, version "89a", 440 x 608
2670386       0x28BF32        Zip archive data, at least v1.0 to extract, compressed size: 112890, uncompressed size: 112890, name: temp.zip
2783320       0x2A7858        End of Zip archive
2783420       0x2A78BC        End of Zip archive
```
發現有.zip檔藏在裡面，所以使用`dd if=Question.gif of=file skip=2670386 bs=1`得到`file`

接下使用`identify -format "%c \n" ../Question.gif`得到
```

724b3052485a6b745062457335797444737142486435504646773d3d
```
將其儲存為`data.txt`。
觀察發現為其為十六進位字串，使用python腳本將其轉換為字串，將結果存在`key.txt`中
```=python
#!/usr/bin/python
# coding = UTF-8
import codecs

files = open("data.txt", "r")
output = open("key.txt", "w+")

for f in files:
    output.write(codecs.decode(f.strip(), 'hex').decode('utf-8') + '\n')

files.close()
output.close()
```
```

```
將結果留下來等等使用。

`unzip file`將`file`解壓縮之後，同樣使用`binwalk file`分析：
```
DECIMAL       HEXADECIMAL     DESCRIPTION
--------------------------------------------------------------------------------
0             0x0             Zip archive data, at least v1.0 to extract, compressed size: 256, uncompressed size: 256, name: partaa.enc
404           0x194           End of Zip archive
426           0x1AA           Zip archive data, at least v1.0 to extract, compressed size: 256, uncompressed size: 256, name: partab.enc
830           0x33E           End of Zip archive
852           0x354           Zip archive data, at least v1.0 to extract, compressed size: 256, uncompressed size: 256, name: partac.enc
1256          0x4E8           End of Zip archive
...
```
發現`temp.zip`裡面也藏有不少.zip檔，每個檔案大小為256KB，使用`split -b 426 temp.zip "file"`將其分離，得到`fileaa, fileab, ..., fileke`

使用`find . -name 'file*' -exec unzip {} \;`大量解壓縮，得到`partaa.enc, ...,  partke.enc`

`key.txt`其實是RSA私鑰，可以用來將所有.enc解密

首先分別在`key.txt`的頭尾加上`-----BEGIN RSA PRIVATE KEY-----`和`-----END RSA PRIVATE KEY-----`，並更改檔名為`key.pem`

使用python腳本進行解密
```
#!/usr/bin/python
# coding=utf-8
import itertools
import os

alphabet = 'abcdefghijklmnopqrstuvwxyz'
for i in itertools.product(alphabet, repeat=2):
    iname = 'part' + ''.join(i) + '.enc'
    oname = 'part2' + ''.join(i) + '.enc'
    if os.path.isfile(iname):
        os.system("openssl rsautl -decrypt -inkey key.pem -in " + iname + " -out " + oname)
```
得到`file2aa, ..., file2ke`

使用`cat file2* > flag`，`binwalk flag`分析得
```
DECIMAL       HEXADECIMAL     DESCRIPTION
--------------------------------------------------------------------------------
0             0x0             JPEG image data, JFIF standard 1.01
```
點開後發現flag為FelicityIsFun